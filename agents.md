...
> 本资料夹存放课堂点子铺旗下的**独立教学工具源代码**。每一个 slug 资料夹都是独立 Git 仓库、独立 Vercel 项目；不要把工具代码搬进 `../eduneo-hub/`。

## 这个资料夹是做什么的

- `eduneo-hub/` 是平台入口：工具目录、DSKP 学习目标搜索、点子许愿池、工具资料登记和缩略图。
...
2. [Hub 蓝图](../eduneo-hub/agents.md) 与 [Hub 交接记录](../eduneo-hub/handoff.md)，了解目前上架与部署状态。
3. [已上架工具覆盖矩阵](../eduneo-hub/docs/published-tools-coverage.md)，先看哪个年级／科目／DSKP 目标仍缺工具。
4. 要按课程自主生成新工具时，先查 `../eduneo-hub/docs/dskp/{tahun}/{subjek}.md` 的已核对摘要；扩充 DSKP 索引前再查术语核对文件：
   - `../eduneo-hub/docs/dskp-bm-glossary-mt-sains.md`
   - `../eduneo-hub/docs/dskp-bm-glossary-others.md`
5. 要让新工具能被「按学习目标找工具」找到，遵守 [DSKP 搜索实施规格](../eduneo-hub/docs/dskp-learning-objective-search.md)。
6. 若工具来自老师的课堂需求，先读 [点子许愿池规格](../eduneo-hub/docs/idea-wish-pool-spec.md)，把工具解决的问题、限制和课堂方式对应回来。
...
- 工具应可单独开发、单独部署与单独回滚；不要引用 `eduneo-hub/` 的运行时代码。
...
新工具上线后，必须回到 `../eduneo-hub/`：

1. 在 `app.js` 的 `TOOLS` 加入真实 `url`、标题、关键词、教学标签、作者、版本和 changelog。
2. 将截图放到 `eduneo-hub/assets/thumbs/{slug}/`，并为每张图写替代文字。
